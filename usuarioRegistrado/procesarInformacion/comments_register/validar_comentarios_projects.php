...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        ...
        $id_proyecto = intval($_POST['projectId']);
        ...
        if (usuario_es_propietario_del_proyecto($conexion, $id_proyecto, $id_usuario)) {
            // Si el usuario actual es propietario del post, permitir comentario
            if (insertar_comentario_register_user_first_type($conexion, $id_proyecto, $id_usuario, $_POST['commentContent'])) {
                echo "trueRegister";
            } else {
                echo "false";
            }
        } else {
            if (insertar_comentario_register_user_second_type($conexion, $id_proyecto, $id_usuario, $_POST['commentContent'])) {
                echo "true";
            } else {
                echo "false";
            }
...
function usuario_es_propietario_del_proyecto($conexion, $id_proyecto, $id_usuario_actual)
{
    $id_usuario_propietario = null;

    // Preparar la consulta para obtener el ID del usuario propietario del post
    $stmt = $conexion->prepare("SELECT id_usuario_proyecto FROM proyectos WHERE id_proyecto = ?");
...
function insertar_comentario_register_user_first_type($conexion, $id_proyecto, $id_usuario_comentario, $contenidoComentario)
{
    // Preparar la consulta con el campo id_estado_comentario
    $stmt_insertar_comentario = $conexion->prepare("INSERT INTO comentarios_proyectos (id_proyecto_comentario, id_usuario_comentario, id_estado_comentario, contenido_comentario) VALUES (?, ?, 1, ?)");

    // Enlazar los parámetros
    $stmt_insertar_comentario->bind_param("iis", $id_proyecto, $id_usuario_comentario, $contenidoComentario);
...
function insertar_comentario_register_user_second_type($conexion, $id_proyecto, $id_usuario_Comentario, $contenidoComentario)
{
    // Preparar la consulta con el campo id_estado_comentario
