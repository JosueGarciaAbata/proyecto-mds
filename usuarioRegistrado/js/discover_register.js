...
$(document).ready(function () {
  ...
  function getPosts() {
    $.ajax({
      ...
      url: "procesarInformacion/comments_register/comments_posts.php",
      type: "POST",
      data: {
        action: "getPostsRegister",
      },
  ...
  function getProjects() {
    $.ajax({
      // Ruta para traer informacion de usuarios  de post no registrados.
      url: "procesarInformacion/comments_register/comments_projects.php",
      type: "POST",
      data: {
        action: "getProjectsRegister",
      },
      success: function (response) {
        console.log(response);
        var data = JSON.parse(response); // Convertir la respuesta JSON en un objeto
        ...
        data.forEach(function (proyecto) {
          ...
          commentIcon.on("click", function () {
            ...
            // Realizar una solicitud AJAX para obtener los datos del post
            $.ajax({
              url: "procesarInformacion/comments_register/obtener_datos_projects.php",
              method: "POST",
              data: {
